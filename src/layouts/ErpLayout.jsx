...
export default function ErpLayout({ company, onReset }) {
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-50 via-white to-slateInk-50 text-slate-900">
      <div className="grid min-h-screen grid-cols-1 md:grid-cols-[280px_1fr]">
        <aside className="border-b border-slate-200/70 bg-white/70 backdrop-blur md:border-b-0 md:border-r">
          ...
          <nav className="px-3 pb-4">
            ...
            <div className="mt-4 border-t border-slate-200/70 pt-4">
              <button
                onClick={onReset}
                className="flex w-full items-center gap-3 rounded-xl px-3 py-2 text-sm text-slate-600 hover:bg-slate-100/70 hover:text-slate-900"
              >
        ...
        <div className="min-w-0">
          <header className="sticky top-0 z-10 border-b border-slate-200/70 bg-white/70 px-4 py-3 backdrop-blur">
            <div className="mx-auto flex max-w-6xl items-center justify-between gap-3">
              <div className="min-w-0">
                <div className="truncate text-sm font-semibold">{company?.siteName || "Site"}</div>
                <div className="mt-0.5 text-xs text-slate-600">Role: {company?.role || "owner"}</div>
              </div>
